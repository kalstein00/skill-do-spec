...
The prepared skill includes Dagu 2.11.2 in `tools/dagu/2.11.2/windows-amd64/dagu.exe` and `tools/dagu/2.11.2/linux-amd64/dagu`, with upstream LICENSE files and a SHA256 manifest. After explicit `--dagu` / `DO_SPEC_DAGU` overrides, the bundled native executable takes priority over PATH and the user cache. A hash mismatch stops execution. Preparing this full distribution uses the developer command `python scripts/bundle_skill.py --with-dagu`; binaries are local distribution artifacts excluded from Git. Linux native execution remains unverified.
